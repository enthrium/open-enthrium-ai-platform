...
Sample `oe-mcp.yaml` configs are the main contribution to this repo. Each sample shows how to connect a real system via OE MCP.

### Step 1 — Create your sample folder

```
server/mcp/samples/your-connector/
└── oe-mcp.yaml    ← config with placeholder credentials + setup comments
```
...
node server/mcp/index.js --stdio server/mcp/samples/your-connector/oe-mcp.yaml

# HTTP serve mode (Cursor / Windsurf)
node server/mcp/index.js --serve server/mcp/samples/your-connector/oe-mcp.yaml
```
...
- Never commit real credentials — use placeholders in `oe-mcp.yaml`
